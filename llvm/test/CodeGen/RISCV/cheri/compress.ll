...
; RUN:   | llvm-objdump --no-print-imm-hex -d -M no-aliases - | FileCheck %s
...
; RUN:   | llvm-objdump --no-print-imm-hex -d -M no-aliases - | FileCheck %s --check-prefix=CHECK-NORVC
...
; RUN:   | llvm-objdump --no-print-imm-hex -d --mattr=+c -M no-aliases - | FileCheck %s
...
; RUN: %riscv64_cheri_purecap_llc -filetype=obj < %t.fnattr \
; RUN:   | llvm-objdump --no-print-imm-hex -d --mattr=+c -M no-aliases - | FileCheck %s --check-prefix=CHECK-NORVC
