...
# WARN-NEXT: >>> defined in {{.*}} is defined as being in section .tdata but the value ({{0xFFFFFFFFFFFFFFF8|0xD8}}) is outside this section. Will create a zero-size capability.
# WARN-NEXT: >>> referenced by <internal>:(.captable+0x0)
# WARN-EMPTY:
# WARN-NEXT: warning: Symbol <unknown kind> _TLS_Data_end
# WARN-NEXT: >>> defined in {{.*}} is defined as being in section .tdata but the value ({{0x1|0xE9}}) is outside this section. Will create a zero-size capability.
...
# DUMP-NO-TDATA-NEXT: OFFSET           TYPE    VALUE
# DUMP-NO-TDATA-NEXT: {{[0-9a-f]+}}    RODATA  fffffffffffffff8 [fffffffffffffff8-fffffffffffffff8]
# DUMP-NO-TDATA-NEXT: {{[0-9a-f]+}}    RODATA  0000000000000001 [0000000000000001-0000000000000001]
# DUMP-NO-TDATA-EMPTY:
