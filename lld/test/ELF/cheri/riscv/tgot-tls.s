# REQUIRES: riscv
# RUN: echo '.tbss; .globl evar; evar: .zero 4' > %t.s

# RUN: %riscv32_cheri_purecap_llvm-mc -cheri-tgot-tls -filetype=obj %t.s -o %t1.32.o
# RUN: ld.lld -z cheri-riscv-v9 -shared -soname=t1.so %t1.32.o -o %t1.32.so

# RUN: %riscv32_cheri_purecap_llvm-mc --defsym PIC=0 -cheri-tgot-tls -filetype=obj %s -o %t.32.o
# RUN: ld.lld -z cheri-riscv-v9 %t.32.o %t1.32.so -o %t.32
# RUN: llvm-readobj -r --cap-relocs %t.32 | FileCheck --check-prefix=RV32-REL %s
# RUN: llvm-readelf -x .got -x .tgot %t.32 | FileCheck --check-prefix=RV32-GOT %s
# RUN: llvm-objdump --no-print-imm-hex -d --no-show-raw-insn %t.32 | FileCheck --check-prefix=RV32-DIS %s

# RUN: %riscv32_cheri_purecap_llvm-mc --defsym PIC=1 -cheri-tgot-tls -filetype=obj %s -o %t.32.pico
# RUN: ld.lld -z cheri-riscv-v9 -shared %t.32.pico %t1.32.so -o %t.32.so
# RUN: llvm-readobj -r --cap-relocs %t.32.so | FileCheck --check-prefix=RV32-SO-REL %s
# RUN: llvm-readelf -x .got -x .tgot %t.32.so | FileCheck --check-prefix=RV32-SO-GOT %s
# RUN: llvm-objdump --no-print-imm-hex -d --no-show-raw-insn %t.32.so | FileCheck --check-prefix=RV32-SO-DIS %s

# RUN: %riscv64_cheri_purecap_llvm-mc -cheri-tgot-tls -filetype=obj %t.s -o %t1.64.o
# RUN: ld.lld -z cheri-riscv-v9 -shared -soname=t1.so %t1.64.o -o %t1.64.so

# RUN: %riscv64_cheri_purecap_llvm-mc --defsym PIC=0 -cheri-tgot-tls -filetype=obj %s -o %t.64.o
# RUN: ld.lld -z cheri-riscv-v9 %t.64.o %t1.64.so -o %t.64
# RUN: llvm-readobj -r --cap-relocs %t.64 | FileCheck --check-prefix=RV64-REL %s
# RUN: llvm-readelf -x .got -x .tgot %t.64 | FileCheck --check-prefix=RV64-GOT %s
# RUN: llvm-objdump --no-print-imm-hex -d --no-show-raw-insn %t.64 | FileCheck --check-prefix=RV64-DIS %s

# RUN: %riscv64_cheri_purecap_llvm-mc --defsym PIC=1 -cheri-tgot-tls -filetype=obj %s -o %t.64.pico
# RUN: ld.lld -z cheri-riscv-v9 -shared %t.64.pico %t1.64.so -o %t.64.so
# RUN: llvm-readobj -r --cap-relocs %t.64.so | FileCheck --check-prefix=RV64-SO-REL %s
# RUN: llvm-readelf -x .got -x .tgot %t.64.so | FileCheck --check-prefix=RV64-SO-GOT %s
# RUN: llvm-objdump --no-print-imm-hex -d --no-show-raw-insn %t.64.so | FileCheck --check-prefix=RV64-SO-DIS %s

# RV32-REL:      .rela.tgot {
# RV32-REL-NEXT:   0x101F0 R_RISCV_CHERI_TLS_TGOT_SLOT evar 0x0
# RV32-REL-NEXT: }
# RV32-REL:      __tgot_cap_relocs {
# RV32-REL-NEXT:   0x101F8 DATA - 0x4 [0x4-0x8]
# RV32-REL-NEXT: }

# RV32-SO-REL:      .rela.dyn {
# RV32-SO-REL-NEXT:   0x24B0 R_RISCV_CHERI_TLS_TGOTREL - 0x0
# RV32-SO-REL-NEXT:   0x24B8 R_RISCV_TLS_DTPMOD32 - 0x0
# RV32-SO-REL-NEXT:   0x24C0 R_RISCV_CHERI_TLS_TGOTREL - 0x8
# RV32-SO-REL-NEXT:   0x24C8 R_RISCV_TLS_DTPMOD32 - 0x0
# RV32-SO-REL-NEXT: }
# RV32-SO-REL:      .rela.tgot {
# RV32-SO-REL-NEXT:   0x248 R_RISCV_CHERI_TLS_TGOT_SLOT evar 0x0
# RV32-SO-REL-NEXT: }
# RV32-SO-REL:      __tgot_cap_relocs {
# RV32-SO-REL-NEXT:   0x250 DATA - 0x4 [0x4-0x8]
# RV32-SO-REL-NEXT: }

# RV32-GOT: section '.tgot':
# RV32-GOT-NEXT: 0x000101f0 00000000 00000000 00000000 00000000
# RV32-GOT: section '.got':
# RV32-GOT-NEXT: 0x000122a0 2c220100 00000000 00000000 00000000
# RV32-GOT-NEXT: 0x000122b0 01000000 00000000 08000000 00000000
# RV32-GOT-NEXT: 0x000122c0 01000000 08000000

# RV32-SO-GOT: section '.tgot':
# RV32-SO-GOT-NEXT: 0x00000248 00000000 00000000 00000000 00000000
# RV32-SO-GOT: section '.got':
# RV32-SO-GOT-NEXT: 0x000024a8 20240000 00000000 00000000 00000000
# RV32-SO-GOT-NEXT: 0x000024b8 00000000 00000000 00000000 00000000
# RV32-SO-GOT-NEXT: 0x000024c8 00000000 08000000

# 0x122b0 - 0x11200 = 0x10b0 (GD evar)
# RV32-DIS:      11200: auipc ca0, 1
# RV32-DIS-NEXT:        cincoffset ca0, ca0, 176

# 0x122a8 - 0x11208 = 0x10a0 (IE evar)
# RV32-DIS:      11208: auipc ca0, 1
# RV32-DIS-NEXT:        lw a0, 160(ca0)

# 0x122c0 - 0x11210 = 0x10b0 (GD lvar)
# RV32-DIS:      11210: auipc ca0, 1
# RV32-DIS-NEXT:        cincoffset ca0, ca0, 176

# 0x124b8 - 0x11218 = 0x10a0 (IE lvar)
# RV32-DIS:      11218: auipc ca0, 1
# RV32-DIS-NEXT:        lw a0, 160(ca0)

# RV32-DIS:      11220: lui a0, 0
# RV32-DIS-NEXT:        cincoffset ca0, ctp, a0
# RV32-DIS-NEXT:        lc ca0, 8(ca0)

# 0x24b8 - 0x1400 = 0x10b8 (GD evar)
# RV32-SO-DIS:      1400: auipc ca0, 1
# RV32-SO-DIS-NEXT:       cincoffset ca0, ca0, 184

# 0x24b0 - 0x1408 = 0x10a8 (IE evar)
# RV32-SO-DIS:      1408: auipc ca0, 1
# RV32-SO-DIS-NEXT:       lw a0, 168(ca0)

# 0x24c8 - 0x1410 = 0x10b8 (GD lvar)
# RV32-SO-DIS:      1410: auipc ca0, 1
# RV32-SO-DIS-NEXT:       cincoffset ca0, ca0, 184

# 0x24c0 - 0x1418 = 0x10a8 (IE lvar)
# RV32-SO-DIS:      1418: auipc ca0, 1
# RV32-SO-DIS-NEXT:       lw a0, 168(ca0)

# RV64-REL:      .rela.tgot {
# RV64-REL-NEXT:   0x10320 R_RISCV_CHERI_TLS_TGOT_SLOT evar 0x0
# RV64-REL-NEXT: }
# RV64-REL:      __tgot_cap_relocs {
# RV64-REL-NEXT:   0x10330 DATA - 0x4 [0x4-0x8]
# RV64-REL-NEXT: }

# RV64-SO-REL:      .rela.dyn {
# RV64-SO-REL-NEXT:   0x2510 R_RISCV_CHERI_TLS_TGOTREL - 0x0
# RV64-SO-REL-NEXT:   0x2520 R_RISCV_TLS_DTPMOD64 - 0x0
# RV64-SO-REL-NEXT:   0x2530 R_RISCV_CHERI_TLS_TGOTREL - 0x10
# RV64-SO-REL-NEXT:   0x2540 R_RISCV_TLS_DTPMOD64 - 0x0
# RV64-SO-REL-NEXT: }
# RV64-SO-REL:      .rela.tgot {
# RV64-SO-REL-NEXT:   0x3B0 R_RISCV_CHERI_TLS_TGOT_SLOT evar 0x0
# RV64-SO-REL-NEXT: }
# RV64-SO-REL:      __tgot_cap_relocs {
# RV64-SO-REL-NEXT:   0x3C0 DATA - 0x4 [0x4-0x8]
# RV64-SO-REL-NEXT: }

# RV64-GOT: section '.tgot':
# RV64-GOT-NEXT: 0x00010320 00000000 00000000 00000000 00000000
# RV64-GOT-NEXT: 0x00010330 00000000 00000000 00000000 00000000
# RV64-GOT: section '.got':
# RV64-GOT-NEXT: 0x00012450 70230100 00000000 00000000 00000000
# RV64-GOT-NEXT: 0x00012460 00000000 00000000 00000000 00000000
# RV64-GOT-NEXT: 0x00012470 01000000 00000000 00000000 00000000
# RV64-GOT-NEXT: 0x00012480 10000000 00000000 00000000 00000000
# RV64-GOT-NEXT: 0x00012490 01000000 00000000 10000000 00000000

# RV64-SO-GOT: section '.tgot':
# RV64-SO-GOT-NEXT: 0x000003b0 00000000 00000000 00000000 00000000
# RV64-SO-GOT-NEXT: 0x000003c0 00000000 00000000 00000000 00000000
# RV64-SO-GOT: section '.got':
# RV64-SO-GOT-NEXT: 0x00002500 f0230000 00000000 00000000 00000000
# RV64-SO-GOT-NEXT: 0x00002510 00000000 00000000 00000000 00000000
# RV64-SO-GOT-NEXT: 0x00002520 00000000 00000000 00000000 00000000
# RV64-SO-GOT-NEXT: 0x00002530 00000000 00000000 00000000 00000000
# RV64-SO-GOT-NEXT: 0x00002540 00000000 00000000 10000000 00000000

# 0x12470 - 0x11340 = 0x1130 (GD evar)
# RV64-DIS:      11340: auipc ca0, 1
# RV64-DIS-NEXT:        cincoffset ca0, ca0, 304

# 0x12460 - 0x11348 = 0x1118 (IE evar)
# RV64-DIS:      11348: auipc ca0, 1
# RV64-DIS-NEXT:        ld a0, 280(ca0)

# 0x12490 - 0x11350 = 0x1140 (GD lvar)
# RV64-DIS:      11350: auipc ca0, 1
# RV64-DIS-NEXT:        cincoffset ca0, ca0, 320

# 0x12480 - 0x11358 = 0x1128 (IE lvar)
# RV64-DIS:      11358: auipc ca0, 1
# RV64-DIS-NEXT:        ld a0, 296(ca0)

# RV64-DIS:      11360: lui a0, 0
# RV64-DIS-NEXT:        cincoffset ca0, ctp, a0
# RV64-DIS-NEXT:        lc ca0, 16(ca0)

# 0x2520 - 0x13d0 = 0x1150 (GD evar)
# RV64-SO-DIS:      13d0: auipc ca0, 1
# RV64-SO-DIS-NEXT:       cincoffset ca0, ca0, 336

# 0x2510 - 0x13d8 = 0x1138 (IE evar)
# RV64-SO-DIS:      13d8: auipc ca0, 1
# RV64-SO-DIS-NEXT:       ld a0, 312(ca0)

# 0x2540 - 0x13e0 = 0x1160 (GD lvar)
# RV64-SO-DIS:      13e0: auipc ca0, 1
# RV64-SO-DIS-NEXT:       cincoffset ca0, ca0, 352

# 0x2530 - 0x13e8 = 0x1148 (IE lvar)
# RV64-SO-DIS:      13e8: auipc ca0, 1
# RV64-SO-DIS-NEXT:       ld a0, 328(ca0)

.global _start
_start:
	clc.tls.gd ca0, evar

	cla.tls.ie a0, evar

	clc.tls.gd ca0, lvar

	cla.tls.ie a0, lvar

.if PIC == 0
	lui a0, %tgot_tprel_hi(lvar)
	cincoffset ca0, ctp, a0, %tgot_tprel_add(lvar)
	clc ca0, %tgot_tprel_lo(lvar)(ca0)
.endif

.tbss
	.zero 4
lvar:
	.zero 4
	.size lvar, . - lvar
