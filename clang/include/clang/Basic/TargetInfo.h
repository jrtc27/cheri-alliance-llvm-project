...
public:
  ...
  virtual bool hasBuiltinAtomic(uint64_t AtomicSizeInBits,
    ...
    // Assume the target supports lock-free atomic operations on single
    // capabilities if it supports them on any integer type.
    if (HasCheriCapabilities && getMaxAtomicInlineWidth() > 0)
      return AtomicSizeInBits == getCHERICapabilityWidth();
